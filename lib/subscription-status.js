export async function checkSubscriptionStatus({ userId }) {
  ...
  try {
    const response = await fetch(
      `${supabaseUrl}/rest/v1/subscriptions?select=status,current_period_end,cancel_at_period_end,stripe_customer_id,stripe_subscription_id,plan,max_users,max_devices_per_user,user_id&user_id=eq.${encodeURIComponent(userId)}&order=current_period_end.desc.nullslast&limit=1`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Supabase subscription lookup failed: ${text}`
      );
    }
